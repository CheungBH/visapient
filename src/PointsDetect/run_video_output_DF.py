    ...
    while True:
        ...
        for i in range(common.CocoPart.Background.value):
            ...
            if cnt%26 is 0:
                ...
                if len(RElbow)==2:
                    file.write("RE's x: " + str(RElbow[0]) + '\n')
                    file.write("RE's y: " + str(RElbow[1]) + '\n')

                if len(RElbow)==2 and len(RShoulder)==2:
                    print("The height of RElbow: "+str(RElbow[1]))
                    print("The height of RShoulder: "+str(RShoulder[1]))
                    file.write("RSHOULDER's x: " + str(RShoulder[0]) + '\n')
                    file.write("RSHOULDER's y: " + str(RShoulder[1]) + '\n')
                    file.write("RE's x: " + str(RElbow[0]) + '\n')
                    file.write("RE's y: " + str(RElbow[1]) + '\n')
                    if  RElbow[1] < RShoulder[1]*1.02:
                        print("Excellent Finish")
                    else:
                        print("Your arm should raise more")
