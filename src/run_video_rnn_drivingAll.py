...
_activation='relu'
_optimizer='Adam'
model_weights_path = "RealTime/model/0213_drivingAll/golf_model.h5"
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                if cnt%130 is 0:
                    ...
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 1:
                        print("Predicted Action:","DFNoFinish")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 2:
                        print("Predicted Action:","DFRight")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 3:
                        print("Predicted Action:","DFTooLow")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 4:
                        print("Predicted Action:","DSRight")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 5:
                        print("Predicted Action:","DSWrongArm")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 6:
                        print("Predicted Action:","DSWrongHand")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 7:
                        print("Predicted Action:","DU1Right")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 8:
                        print("Predicted Action:","DU1WrongArm")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 9:
                        print("Predicted Action:","DU2NoTurn")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 10:
                        print("Predicted Action:","DU2Right")
                    if np.argmax(test_sample_y_hat, axis=1)[0] == 11:
